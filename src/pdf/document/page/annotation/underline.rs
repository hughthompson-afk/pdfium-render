...
impl<'a> PdfPageUnderlineAnnotation<'a> {
    ...
    /// Generates and sets an appearance stream for this underline annotation using the Normal appearance mode.
    ///
    /// This method builds a PDF content stream that renders horizontal lines at the bottom of each
    ...
    /// Generates and sets an appearance stream for this underline annotation with a specific appearance mode.
    ///
    /// This method builds a PDF content stream that renders horizontal lines at the bottom of each
    /// attachment point using the annotation's stroke color, then sets it as the annotation's appearance stream.
    /// This is required for the annotation to display and flatten correctly.
    ...
    pub fn generate_appearance_stream_with_mode(&mut self, mode: PdfAppearanceMode) -> Result<(), PdfiumError> {
        #[cfg(target_arch = "wasm32")]
        {
            use web_sys::console;
            console::log_1(&"═══════════════════════════════════════════════════════════".into());
            console::log_1(&"🔧 PdfPageUnderlineAnnotation::generate_appearance_stream_with_mode()".into());
            console::log_1(&"═══════════════════════════════════════════════════════════".into());
        ...
        let stroke_width = 1.0; // Underline typically uses a fixed width

        // Get attachment points (quadpoints) - underline uses these if available
        ...
        let content_stream_result = self.build_underline_appearance_stream(mode, attachment_points, rect, stroke_color, stroke_width);
    ...
    /// Builds a PDF content stream for underline annotation appearance.
    ///
    /// This method creates a content stream that draws horizontal lines at the bottom of each attachment point
    /// using the specified stroke color and width.
    fn build_underline_appearance_stream(
        &self,
        ...
        {
            use web_sys::console;
            console::log_1(&"📐 build_underline_appearance_stream() - Building content stream".into());
        ...
        // Draw horizontal lines at the bottom of each attachment point, or use rect as fallback
        if attachment_points.len() > 0 {
            for i in 0..attachment_points.len() {
                if let Ok(quad) = attachment_points.get(i) {
                    // Underline is drawn at the bottom of the quadpoint
                    let y_bottom = quad.bottom().value - bottom;
                    let x_left = quad.left().value - left;
                    let x_right = quad.right().value - left;

                    // Draw horizontal line from left to right at bottom Y coordinate
                    stream.push_str(&format!("{:.4} {:.4} m\n", x_left, y_bottom)); // moveto left
                    stream.push_str(&format!("{:.4} {:.4} l\n", x_right, y_bottom)); // lineto right
                    stream.push_str("S\n"); // stroke

                    #[cfg(target_arch = "wasm32")]
                    {
                        use web_sys::console;
                        console::log_1(&format!("   Drew underline {}: x={:.2} to x={:.2} at y={:.2}",
                            i, x_left, x_right, y_bottom).into());
                    }
                }
            }
        } else {
            ...
            // Draw horizontal line at bottom of annotation rect
            stream.push_str("0 0 m\n"); // moveto left bottom
            stream.push_str(&format!("{:.4} 0 l\n", width)); // lineto right bottom
            stream.push_str("S\n"); // stroke

            #[cfg(target_arch = "wasm32")]
            {
                use web_sys::console;
                console::log_1(&format!("   Drew underline using rect fallback: x=0 to x={:.2} at y=0",
                    width).into());
            }
        }
