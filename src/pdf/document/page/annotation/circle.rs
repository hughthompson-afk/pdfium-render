...
impl<'a> PdfPageCircleAnnotation<'a> {
    ...
    /// Returns the stroke width of this circle annotation.
    ///
    ...
    pub fn stroke_width(&self) -> Result<f32, PdfiumError> {
        ...
        {
            use web_sys::console;
            console::log_1(&"🔍 PdfPageCircleAnnotation::stroke_width() - Starting".into());
        }
    ...
    /// Sets the stroke width of this circle annotation.
    ///
    ...
    pub fn set_stroke_width(&mut self, width: f32) -> Result<(), PdfiumError> {
        ...
        {
            use web_sys::console;
            console::log_1(&format!("🔧 PdfPageCircleAnnotation::set_stroke_width() - width: {:.4}", width).into());
        }
    ...
    /// Sets the rectangle of this circle annotation using an appearance stream.
    ///
    /// This method builds a PDF content stream that draws the circle/ellipse and sets it as the
    /// annotation's appearance stream. The circle will be drawn using the annotation's
    ...
    /// Sets the rectangle of this circle annotation with a specific appearance mode.
    pub fn set_rect_with_mode(
        ...
        let content_stream = self.build_circle_appearance_stream()?;
    ...
    /// Builds the PDF content stream string for drawing the circle/ellipse.
    ///
    /// PDF doesn't have a native circle operator, so we approximate an ellipse using
    /// cubic bezier curves. The ellipse is drawn using 4 bezier curves.
    fn build_circle_appearance_stream(&self) -> Result<String, PdfiumError> {
        ...
        // Calculate center and radii (relative to the original shape size)
        let cx = width / 2.0;
        let cy = height / 2.0;
        let rx = width / 2.0;
        let ry = height / 2.0;
        ...
        {
            use web_sys::console;
            console::log_1(&"📏 build_circle_appearance_stream() - Getting stroke width".into());
        }
        ...
        // Magic number for approximating a circle with bezier curves: 4/3 * (sqrt(2) - 1) ≈ 0.55228475
        let k = 0.55228475;
        let kx = k * rx;
        let ky = k * ry;
        ...
        // Fourth curve: bottom to right (closes the path)
        stream.push_str(&format!(
            "{:.4} {:.4} {:.4} {:.4} {:.4} {:.4} c\n",
            cx + kx, cy - ry,
            cx + rx, cy - ky,
